...
def analyze_test_failures(error_message: str) -> Dict[str, Any]:
    ...
    # Анализируем конкретные паттерны ошибок тестов
    if "7 != 9" in error_message and "7 != 5" in error_message:
        suggestions = [
            "Ваша функция всегда возвращает одно и то же значение (7) для разных входных данных.",
            "Убедитесь, что ваша функция правильно обрабатывает входные аргументы и выполняет необходимые вычисления.",
            "Проверьте, что вы не используете жестко заданное значение вместо вычисления результата.",
            "Добавьте отладочный вывод, чтобы увидеть, какие значения обрабатывает ваша функция."
        ]
    
    if "1 != -1" in error_message:
        suggestions.append("Обратите внимание на знак результата. Возможно, вам нужно изменить знак возвращаемого значения.")
    
    if "2 != 0" in error_message or "3 != 0" in error_message:
        suggestions.append("Ваша функция возвращает входное значение, когда должна возвращать 0. Проверьте условия задачи.")
    
    # Проверяем наличие ключевых слов FFFF (все тесты не прошли)
    if "FFFF" in error_message and len(failures) >= 3:
        suggestions = [
            "Все тесты не прошли, это может означать, что ваша функция содержит фундаментальную ошибку в логике.",
            "Проверьте, что вы правильно поняли условие задачи и требования к функции.",
            "Убедитесь, что ваша функция выполняет вычисления на основе входных данных, а не возвращает константы.",
            "Добавьте отладочные выводы (print) внутри функции, чтобы увидеть, как обрабатываются данные на каждом шаге."
        ]
...
def format_analysis_for_display(analysis: Dict[str, Any]) -> str:
    ...
    if analysis.get("error_type"):
        result += f"<strong>{analysis['error_type']}</strong><br>\n"
    
    if analysis.get("explanation"):
        result += f"<strong>Объяснение:</strong><br>\n{analysis['explanation'].replace('\n', '<br>')}<br>\n\n"
    ...
    # Если ничего не добавлено (что маловероятно), вернем базовое сообщение
    if not result:
        result = "Попробуйте внимательно проверить ваш код на наличие логических ошибок."
